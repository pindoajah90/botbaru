...
const start = (pakforlay = new Client()) => {
	...
	await pakforlay.sendText(chat.id, `Sorry, grup pada bot ini sudah penuh\nMaximal grupnya: ${groupLimit}`).then(() => {
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
	    ...
	    await pakforlay.sendText(chat.id, `Sorry, bot tidak akan join jika member grup tidak melebihi ${memberLimit} orang`).then(() => {
	      pakforlay.leaveGroup(chat.id)
	      pakforlay.deleteChat(chat.id)
	    })
	    } else {
        await pakforlay.simulateTyping(chat.id, true).then(async () => {
          await pakforlay.sendText(chat.id, `Hai minna~, Saya BOTOL (Bot Online). Untuk mengetahui semua fitur silahkan ketik ${prefix}menu`)
        })
	    }
	}
    })
