            ...
            if (isOwnerBot) {
                await pakforlay.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await pakforlay.sendText(chekgrup.id, `Hai minna~, Saya BOTOL (Bot Online). Untuk mengetahui semua fitur silahkan ketik ${prefix}menu`)
                      })
            } else {
                let cgrup = await pakforlay.getAllGroups()
                if (cgrup.length > groupLimit) return pakforlay.reply(from, `Sorry, grup pada bot ini sudah penuh\nMaximal grupnya: ${groupLimit}`, id)
                if (cgrup.size < memberLimit) return pakforlay.reply(from, `Sorry, bot tidak akan join jika member grup tidak melebihi ${memberLimit} orang`, id)
                await pakforlay.joinGroupViaLink(linkgrup)
                      .then(async () =>{
